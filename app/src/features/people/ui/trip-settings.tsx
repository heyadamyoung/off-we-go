...
import type { Device, Invite, Person, Toast, Trip } from '../../../shared/model/types'
    ...
    <Sheet wide title="Trip settings" onClose={props.onClose}
      ...
      {props.tab === 'phones' && (
        <PhonesTab tripId={props.tripId} family={props.family} canEdit={props.canEdit}
                   me={props.me} toast={props.toast} phones={props.phones} onChange={props.onPhones} />
      )}
    </Sheet>
  )
}
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="field">Who&apos;s going
          <input value={fields.crew} placeholder="e.g. Adam and Catherine"
...
function PeopleTab({ tripId, family, me, appLink, toast, trip }: SettingsProps) {
  ...
  const add = async event => {
    ...
    } catch (error) { toast(appErrorMessage(error, 'send-invite'), 'error') }
  ...
  const revoke = async (id = '') => {
    ...
    } catch (error) { toast(appErrorMessage(error, 'remove-invite'), 'error') }
  }

  const remove = async (person: Person) => {
    if (!window.confirm(
      `Remove ${person.name} from this trip? Their registered phones will stop reporting.`)) return
    try {
  ...
  return (
    <>
      <div>
        {family.map(person => (
          <div key={person.id} className="flex items-center gap-3 border-b border-line py-2.5 last:border-b-0">
            <span className={'avatar size-8 ' + (person.memberRole === 'viewer' ? 'plain' : 'bg-[#5B8DEF]')}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className="text-xs text-muted">
                {person.handle && (
                  <Link to="/users/$handle" params={{ handle: person.handle }}>@{person.handle}</Link>
                )}
                {person.handle ? ' · ' : ''}
            ...
            <span className="rounded-md border border-line px-2 py-1.5 text-[11px] font-bold text-faint">
              {person.memberRole === 'owner' ? 'Owner' : person.memberRole === 'editor' ? 'Editor' : 'Viewer'}
            </span>
            {canManage && person.id !== me.id && (
              <button className="rounded-lg px-2 py-1.5 text-xs font-bold text-faint hover:bg-raised2
                                 hover:text-danger" onClick={() => remove(person)}>Remove</button>
            )}
          </div>
        ))}
        {pending.map(invite => (
          <div key={invite.id}
            ...
            <span className="avatar plain size-8">{(invite.name || invite.email || '?')[0].toUpperCase()}</span>
            <div className="min-w-0 flex-1">
              <b className="block text-[13.5px]">{invite.name || invite.email}</b>
              <span className="text-xs text-muted">Invited — not signed in yet</span>
            </div>
            {canManage && (
              <button className="grid size-7 place-items-center rounded-lg text-faint hover:text-danger"
                      onClick={() => revoke(invite.id)} title="Cancel invitation">
                <Icon n="x" s={13} />
              </button>
            )}
          </div>
      ...
      {canManage ? (
        <>
          <p className="hint">
            Everyone signs in, including people just following along. Invite them by the email address
            they will use — that is what grants access.
          </p>
          ...
          <p className="hint">
            ...
            <a href={`mailto:safety@threadway.ai?subject=${encodeURIComponent('Off We Go safety concern')}` +
