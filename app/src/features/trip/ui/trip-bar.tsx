  ...
  return (
    <button className={'card fcard flex w-[172px] flex-none flex-col overflow-hidden rounded-xl border ' +
      ...
      (selected ? 'border-accent shadow-[0_0_0_1px_var(--c-accent)]' : 'border-line hover:border-line2')}
            onClick={onSelect}>
      <div className="relative h-[78px] overflow-hidden bg-raised2">
        ...
        <span className={'absolute left-1.5 top-1.5 rounded border px-1.5 py-0.5 text-[9px] font-extrabold ' +
          'uppercase tracking-[.1em] ' + (item.status === 'now'
      ...
      <div className="flex flex-col gap-0.5 px-2.5 pb-2 pt-1.5">
        <div className="t line-clamp-2 text-[12.5px] font-bold leading-tight">{item.title}</div>
        <div className="truncate text-[11px] text-faint">{item.meta}</div>
      </div>
    </button>
  )
}
