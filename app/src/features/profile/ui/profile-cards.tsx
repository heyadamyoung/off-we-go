...
import {
  DISCOVERABILITY, NOTIFICATIONS, toggleChannel,
  type Channel, type Discoverability, type NotificationChoice, type NotificationKey,
  type Preferences,
} from '../../../preferences-core'

export function Card({ title, aside, children, wide }:
  { title: string; aside?: ReactNode; children: ReactNode; wide?: boolean }) {
  return (
...
/* A row of label, sub-label and one action — the shape most of this page is
   made of. */
export function Row({ lead, title, detail, action }:
  { lead?: ReactNode; title: ReactNode; detail?: ReactNode; action?: ReactNode }) {
  return (
...
export function NotificationsCard({ preferences, onChange }:
  { preferences: Preferences; onChange: (next: Preferences) => void }) {
      ...
      {NOTIFICATIONS.map(item => {
        ...
        return (
          <label key={item.key} className="sw border-b border-line last:border-b-0">
            <input type="checkbox" checked={choice.on}
            ...
            <div className="flex gap-1.5">
              {item.channels.map(channel => (
                <button key={channel} type="button" disabled={!choice.on}
                        onClick={event => {
                          event.preventDefault()
                          set(item.key, toggleChannel(choice, channel as Channel))
                        }}
                        className={'rounded px-1.5 py-0.5 text-[10.5px] font-bold uppercase tracking-[.06em] ' +
                          (choice.on && choice.channels.includes(channel as Channel)
                            ? 'bg-accent-soft text-accent' : 'bg-raised2 text-muted') +
...
export function PrivacyCard({ preferences, onChange }:
  { preferences: Preferences; onChange: (next: Preferences) => void }) {
  ...
  const toggle = (key: 'showLivePosition' | 'keepTrail' | 'stripPhotoLocation',
      ...
      <div className="flex flex-col gap-1.5">
        {DISCOVERABILITY.map(option => (
          <label key={option.value}
                 className={'flex cursor-pointer items-center gap-2.5 rounded-lg border px-2.5 py-2 text-[13px] ' +
                   (privacy.discoverable === option.value
                     ? 'border-accent bg-accent-soft' : 'border-line')}>
            <input type="radio" name="discoverable" className="sr-only"
                   checked={privacy.discoverable === option.value}
                   onChange={() => set({ discoverable: option.value as Discoverability })} />
            <i className={'size-3.5 flex-none rounded-full ' + (privacy.discoverable === option.value
              ? 'border-4 border-accent' : 'border-[1.5px] border-line2')} />
            {option.label}
          </label>
        ))}
      </div>
      {toggle('showLivePosition', "Show my live position on trips I'm travelling",
        'Followers see the marker; nobody else does')}
      {toggle('keepTrail', 'Keep my GPS trail after a trip ends',
