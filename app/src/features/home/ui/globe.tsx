...
const BLUE_MARBLE = 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/' +
...
const STYLE: any = {
  ...
  sources: {
    world: {
      type: 'raster', tiles: [BLUE_MARBLE], tileSize: 256, minzoom: 0, maxzoom: 8,
...
const Globe = memo(function Globe({ places = [], home, live, waiting }: GlobeProps) {
  ...
  useEffect(() => {
    ...
    return () => { created.remove(); setMap(null) }
  }, [])
  ...
  useEffect(() => {
    ...
    const add = () => {
      ...
      map.addLayer({
        ...
        paint: {
          'line-color': ACCENT, 'line-width': 2, 'line-opacity': 0.6,
          'line-dasharray': [1.5, 3.5],
        },
      })
      map.addLayer({
        id: 'walked-glow', type: 'line', source: 'walked',
