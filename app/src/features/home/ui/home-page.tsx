...
export default function HomePage() {
  const { trips, invites, profile, loading, error, reload } = useLanding()
  ...
  if (loading) return <HomeShell me={profile} waiting><span /></HomeShell>

  return (
    <HomeShell me={profile} places={scene.places} home={scene.home} live={scene.live}
    ...
    <>
      ...
      <div className="mt-1.5 flex flex-col border-t border-line">
        {STEPS.map((step, index) => (
          <div key={step.title} className="flex items-start gap-4 border-b border-line py-3.5">
            <i className="mt-px grid size-[26px] flex-none place-items-center rounded-full border-[1.5px]
                          border-line2 text-xs font-extrabold not-italic text-muted">{index + 1}</i>
            <div>
      ...
      <div className="flex items-center gap-2.5">
        <Link className="btn btn-accent px-5 py-3 text-sm" to="/new">
          <Icon n="plus" s={14} />Start your first trip
        </Link>
        {invites > 0 && (
  ...
  return (
    <>
      <div className="flex items-center gap-2.5 text-xs font-bold uppercase tracking-[.14em] text-accent">
        ...
        {progress.state === 'live' ? 'Live now' : progress.state === 'upcoming' ? 'Coming up' : 'Most recent'}
        {progress.days > 0 && progress.state === 'live' && (
          <span className="tracking-[.06em] text-faint">· Day {progress.day} of {progress.days}</span>
        )}
      </div>
      <h1 className="m-0 text-[56px] font-extrabold leading-[.98] tracking-[-.03em] text-balance">
        {trip.title}
      ...
      <p className="m-0 max-w-[480px] text-base leading-relaxed text-muted">
        ...
        {counted ? <>. <b className="font-semibold text-ink">{counted}</b>.</> : '.'}
      </p>
      <div className="flex items-center gap-2.5">
        <Link className="btn btn-accent px-5 py-3 text-sm" to="/trips/$slug" params={{ slug: trip.slug }}>
          Open the trip
        </Link>
        <Link className="btn btn-ghost px-5 py-3 text-sm" to="/trips/$slug"
              params={{ slug: trip.slug }} search={{ sheet: 'add' as const }}>
          <Icon n="camera" s={14} />Add photos
        </Link>
      </div>
      <div className="mt-1 flex flex-col border-t border-line">
        <MoreLink to="/new" icon="plus" title="Start a new trip"
                  detail="Plan the next one while this one is still going." />
        <MoreLink to="/invitations" icon="people" title="Invitations"
                  detail="Trips you've been asked to join or follow."
                  note={invites ? `${invites} waiting` : undefined} />
        <MoreLink to="/past" icon="trips" title="Past trips"
