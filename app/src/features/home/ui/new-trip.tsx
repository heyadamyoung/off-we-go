...
export default function NewTripPage({ step }: { step: number }) {
  ...
  const create = async (thenLinkPhone: boolean) => {
    ...
    try {
      ...
      for (const guest of guests) {
        try {
          await invitePerson(trip.id || '', { email: guest.email, role: guest.role })
        } catch {
  ...
  return (
    <HomeShell me={profile} wide waiting home={scene.home} places={scene.places}>
      ...
      <div className="flex items-center gap-1.5">
        {STEPS.map(([number, label], index) => (
          <span key={number} className="flex items-center gap-1.5">
            ...
            <span
              className={
                'flex items-center gap-2 rounded-full border px-2.5 py-1.5 pl-1.5 text-xs font-bold ' +
      ...
      {step === 1 && (
        <>
          <section className="surface flex flex-col gap-3 p-[18px]">
            ...
            {span && (
              <p className="hint">
                {formatRange(fields.startsOn, fields.endsOn)} · {span} days
              </p>
            )}
            <p className="hint">
              Home base sets the &ldquo;left home&rdquo; and &ldquo;back home&rdquo; ends of the arc
              on your globe.
            </p>
          </section>
      ...
      {step === 2 && (
        <People guests={guests} setGuests={setGuests} onBack={() => go(1)} onNext={() => go(3)} />
      )}
