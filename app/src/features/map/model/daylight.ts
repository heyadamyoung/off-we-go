...
function sunAltitude(date: Date, lat: number, lng: number) {
...
function daylightAt(date: Date, lngLat: Coordinates) {
  const alt = sunAltitude(date, lngLat[1], lngLat[0])
  return { alt, base: alt > LIGHT_ABOVE ? ('light' as const) : ('dark' as const), ...tintFor(alt) }
}

// The sun moves about a quarter of a degree a minute, so a minute is plenty.
function useDaylight(lngLat: Coordinates | null | undefined, everyMs = 60000) {
  const [at, setAt] = useState(() => Date.now())
  useEffect(() => {
    const id = setInterval(() => setAt(Date.now()), everyMs)
    return () => clearInterval(id)
  }, [everyMs])
  const lng = lngLat ? lngLat[0] : 0,
    lat = lngLat ? lngLat[1] : 0
