...
function metres(a: Coordinates, b: Coordinates) {
  const R = 6371000,
    r = (d: number) => (d * Math.PI) / 180
  const dLat = r(b[1] - a[1]),
    dLng = r(b[0] - a[0])
  const h =
    Math.sin(dLat / 2) ** 2 + Math.cos(r(a[1])) * Math.cos(r(b[1])) * Math.sin(dLng / 2) ** 2
  return 2 * R * Math.asin(Math.sqrt(h))
}

function validLngLat(lng: number | null | undefined, lat: number | null | undefined) {
  return (
    Number.isFinite(lng) &&
    Number.isFinite(lat) &&
...
function coordinateLabel([lng, lat]: Coordinates, precision = 4) {
  return (
    `${Math.abs(lat).toFixed(precision)}° ${lat >= 0 ? 'N' : 'S'}, ` +
    `${Math.abs(lng).toFixed(precision)}° ${lng >= 0 ? 'E' : 'W'}`
  )
}
function routeKm(coords: Coordinates[]) {
  let d = 0
...
function trailKm(fixes: WalkedFix[]) {
  const by = new Map<string, WalkedFix[]>()
  for (const f of fixes) {
    if (f.accuracy != null && f.accuracy > 80) continue
    const key = String(f.deviceId ?? '')
  ...
  for (const list of by.values()) {
    ...
    for (let i = 1; i < list.length; i++) {
      ...
      const dt = (list[i].at.getTime() - list[i - 1].at.getTime()) / 1000
...
const lineOf = (coords: Coordinates[]): Feature<LineString> => ({
  type: 'Feature',
  properties: {},
