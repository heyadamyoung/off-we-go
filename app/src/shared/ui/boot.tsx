...
export default function Boot({ what = 'your trips', error, action = 'load-trip', onRetry }: BootProps) {
  if (!error) {
    return (
      <Screen>
  ...
  return (
    <Screen>
      ...
      {onRetry && <button className="btn btn-ghost mt-1" onClick={onRetry}>Try again</button>}
    </Screen>
  )
}
