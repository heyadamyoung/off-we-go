...
2. Increment `versionCode` (and update `versionName` when appropriate) in `android/app/build.gradle`, then build with JDK 21 or newer and Android SDK 35: `VITE_API_URL=https://wayfare.threadway.ai/api pnpm android:sync`.
...
6. Confirm `https://wayfare.threadway.ai/.well-known/assetlinks.json` lists `ai.threadway.wayfare` and that fingerprint before testing the OIDC browser return to the app.
...
8. Complete Google Play’s Data safety and foreground-service declarations using Wayfare’s published privacy policy, then upload the AAB to internal testing before production.
