...
test('an owner can update trip details and their global profile', async () => {
  ...
  const loaded = await app.inject({
    method: 'GET',
    url: `/api/trips/current?t=${otherTrip.slug}`,
    headers: { authorization },
  })
...
test('an owner can list and revoke a phone', async () => {
  ...
  assert.equal(
    (
      await app.inject({
        method: 'DELETE',
        url: `/api/trips/${trip.id}/devices/${device.id}`,
        headers: { authorization },
      })
    ).statusCode,
    204,
...
test('an owner can upload a private avatar and attach it to their global profile', async () => {
  ...
  const { app, authorization } = await setup({
    async storeAvatar(input) {
      stored.push(input)
      const suffix =
        stored.length === 1 ? `${input.profileId}-legacy.jpg` : `${input.profileId}.jpg`
  ...
  replacement.append(
    'avatar',
    new Blob([Buffer.from('replacement-image')], { type: 'image/jpeg' }),
    'me-again.jpg',
  )
