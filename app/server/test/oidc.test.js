...
test('the OIDC adapter uses discovery, PKCE, state and nonce to return a verified identity', async () => {
  assert.ok(moduleUnderTest?.createOidcIdentityProvider, 'the OIDC identity provider has not been implemented')
  ...
  const oidc = {
    ...
    async authorizationCodeGrant(config, currentUrl, checks) {
      ...
      return { claims: () => ({
        iss: 'https://identity.example.com/oidc', sub: 'subject-1',
  ...
  assert.deepEqual(identity, {
    issuer: 'https://identity.example.com/oidc', subject: 'subject-1',
