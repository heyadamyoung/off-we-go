...
test('PostgreSQL migrations create a repository that persists auth, trips and GPS', async t => {
  ...
  const handleReservationTable = await schema.query(`select to_regclass('public.profile_handle_reservations') name`)
  ...
  for (const name of ['id', 'handle', 'display_name', 'avatar_path']) assert.equal(profileColumnNames.has(name), true)
  ...
  assert.equal(await repository.reserveProfileHandle({
    reservationHash: 'owner-handle-reservation', handle: 'prairie-adam', expiresAt,
  }), true)
  ...
  assert.equal(await repository.reserveProfileHandle({
    reservationHash: 'taken-handle-reservation', handle: 'prairie-adam', expiresAt,
  }), false)
  assert.equal(await repository.reserveProfileHandle({
    reservationHash: 'new-handle-reservation', handle: 'new-traveller', expiresAt,
  }), true)
  ...
  await aliases.query('insert into trip_slug_aliases(trip_id,slug) values($1,$2)', [trip.id, 'persistent-trip-a1b2c3'])
  await aliases.end()
  ...
  assert.equal(await repository.reserveProfileHandle({
    reservationHash: 'friend-handle-reservation', handle: 'friendly-alex', expiresAt,
  }), true)
...
test('the human-slug migration upgrades existing profiles and preserves old trip links', async t => {
  ...
  for (const name of files.filter(name => name < '008_global_profiles.sql')) {
    ...
    await client.query('insert into schema_migrations(name,checksum) values($1,$2)', [
      name, moduleUnderTest.migrationChecksumStatus(sql).canonicalChecksum,
    ])
  }
  ...
  await client.query(`insert into trips(id,slug,title,day_count) values($1,$2,$3,1)`,
    [tripId, 'scotland-2027-a1b2c3', 'Scotland 2027'])
  await client.query(`insert into trip_members(trip_id,user_id,role,display_name)
    values($1,$2,'owner','Alex Owner')`, [tripId, userId])
  const profilesSql = await readFile(join(migrationsDirectory, '008_global_profiles.sql'), 'utf8')
  await client.query(profilesSql)
  await client.query('insert into schema_migrations(name,checksum) values($1,$2)', [
    '008_global_profiles.sql', moduleUnderTest.migrationChecksumStatus(profilesSql).canonicalChecksum,
  ])
  await client.end()

  const repository = await moduleUnderTest.createPostgresRepository({
    databaseUrl, adminEmail: 'alex@example.com',
  })
  t.after(() => repository.close())
  await repository.migrate()
