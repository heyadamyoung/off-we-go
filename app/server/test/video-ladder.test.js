...
/* Side by side rather than one after another: every case stands its own
   server up on its own port with its own films, and most of each is ffmpeg
   encoding on cores the others are not using. In a row this file was
   twenty-five seconds and the whole server suite waited on it. */
describe('the ladder', { concurrency: true }, () => {
