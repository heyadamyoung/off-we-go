...
test('an invited user can complete OIDC authorization code with PKCE and exchange a one-time app handoff', async () => {
  ...
  assert.equal(
    ...
    'https://offwego.example.com/api/auth/oidc/callback',
  )
  ...
  const callback = await app.inject({
    method: 'GET',
    url: `/api/auth/oidc/callback?code=provider-code&state=${encodeURIComponent(state)}`,
  })
  ...
  const exchanged = await app.inject({
    ...
    headers: { cookie: loginCookie(started) },
    payload: { token: handoff.searchParams.get('token') },
  })
...
test('a cancelled native provider flow returns an actionable error to the app', async () => {
  ...
  const started = await app.inject({
    method: 'GET',
    url: `/api/auth/oidc/start?client=native&challenge=${challenge}`,
  })
  const state = new URL(started.headers.location).searchParams.get('state')
  const callback = await app.inject({
    method: 'GET',
...
test('a linked OIDC subject keeps the same Off We Go account after its provider email changes', async () => {
  ...
  const app = await moduleUnderTest.buildServer({
    repository: createMemoryRepository({ allowedEmails: ['owner@example.com'] }),
    mailer: { async send() {} },
    identityProvider: createOidcProvider(identity),
    publicUrl: 'https://offwego.example.com',
    sessionSecret: 'test-secret-that-is-long-enough',
  })
  const signIn = async () => {
...
test('native OIDC sign-in returns through the app universal-link handoff', async () => {
  ...
  const stolen = await app.inject({
    ...
    url: '/api/auth/exchange',
    payload: {
      token: destination.searchParams.get('token'),
      client: 'native',
      verifier: 'wrong-device-verifier',
    },
  })
...
test('OIDC logout ends the provider session and uses a fixed safe return', async () => {
  ...
  const web = await app.inject({
    method: 'GET',
    url: '/api/auth/oidc/logout?return=https://evil.example',
  })
  assert.equal(web.statusCode, 302)
  assert.equal(
    new URL(web.headers.location).searchParams.get('post_logout_redirect_uri'),
    'https://offwego.example.com/',
  )
  const native = await app.inject({ method: 'GET', url: '/api/auth/oidc/logout?client=native' })
  assert.equal(
    new URL(native.headers.location).searchParams.get('post_logout_redirect_uri'),
...
test('custom password sign-in completes the Logto interaction without exposing hosted Logto pages', async () => {
  ...
  const experienceFetch = async (url, options = {}) => {
    ...
    if (path === '/oidc/auth') {
      return new Response(null, {
        ...
        headers: {
          location: '/sign-in',
          'set-cookie': 'logto_interaction=secret; Path=/; HttpOnly',
        },
      })
    }
  ...
  const calls = [
    ...
    [
      'POST',
      '/api/auth/experience/verification/password',
      {
        identifier: { type: 'email', value: 'owner@example.com' },
  ...
  for (const [method, url, payload] of calls) {
    const response = await app.inject({
      method,
      url,
      headers: { 'x-wayfare-experience': started.json().interaction },
      payload,
    })
  ...
  const submitted = await app.inject({
    method: 'POST',
    url: '/api/auth/experience/submit',
    headers: { 'x-wayfare-experience': started.json().interaction },
    payload: {},
  })
  ...
  assert.equal(
    upstream.some(call => new URL(call.url).pathname === '/sign-in'),
    false,
  )
...
test('custom account creation sends a Logto verification code without requiring a trip invitation', async () => {
  ...
  await app.inject({
    method: 'PUT',
    url: '/api/auth/experience',
    headers: { cookie },
    payload: { interactionEvent: 'Register' },
  })
  const sent = await app.inject({
    method: 'POST',
    url: '/api/auth/experience/verification/verification-code',
    headers: { cookie },
    payload: {
      identifier: { type: 'email', value: 'stranger@example.com' },
...
test('account creation reserves a normalized unique profile handle before contacting Logto', async () => {
  ...
  const reserve = interaction =>
    app.inject({
      method: 'POST',
      url: '/api/auth/experience/handle',
      headers: { 'x-wayfare-experience': interaction },
      payload: { handle: 'Adam-Young' },
    })
  ...
  assert.equal(
    upstreamHandleRequests,
    0,
    'private handle reservations must never be forwarded to Logto',
  )

  const existing = await repository.ensureUser('existing@example.com')
  assert.deepEqual(
    await repository.updateProfile(existing, { handle: 'adam-young' }),
    { conflict: 'handle' },
  ...
  const forged = await app.inject({
    method: 'POST',
    url: '/api/auth/experience/handle',
    headers: { 'x-wayfare-experience': 'not-an-issued-registration' },
...
test('account creation rejects malformed and impersonating profile handles', async () => {
  const experienceFetch = async url =>
    new URL(url).pathname === '/oidc/auth'
      ? new Response(null, {
          status: 303,
          headers: {
            location: '/sign-in',
...
test('account creation binds the reserved handle to the new global profile', async () => {
  ...
  const calls = [
    ...
    [
      'POST',
      '/api/auth/experience/verification/verification-code',
      {
    ...
    [
      'POST',
      '/api/auth/experience/verification/verification-code/verify',
      {
    ...
    [
      'POST',
      '/api/auth/experience/profile',
      { type: 'password', value: 'a sufficiently long password' },
    ],
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    statuses.push(
      (
        await app.inject({
          method: 'POST',
          url: '/api/auth/experience/verification/password',
          headers: { 'x-wayfare-experience': interaction },
