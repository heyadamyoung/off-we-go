...
/* The rule is unit-tested next door, against points and arrays. This is the
   ...
   And that filing it leaves its coordinates alone, which is the distinction
   the whole of this went wrong over. A picture taken near the museum is filed
   at the museum — that link is what gives the gallery its shape — but where it
   was taken is still where it was taken. For a while the map drew a stop's
   photographs as one stack on the stop's own pin, so the street outside, the
   bikes, the family and the sky collapsed onto it; unfiling everything stopped
   that and cost the gallery its shape, and drawing them at their own point
   stopped it properly. Filing and placing are two questions, and the answer to
   the second is never written here. */
...
test('a photograph taken at a stop is filed there without being told', async t => {
  ...
  /* And the point it was taken at is untouched by that. This is the line the
     bug lived on: the coordinates were never overwritten, only overruled on
     the way to the screen, and a filing that moved them would be the thing
     everybody thought was happening. */
  assert.equal(photo.lng, 4.8852)
  assert.equal(photo.lat, 52.36)
  assert.equal(photo.locationSource, 'exif', 'still its own GPS, not the stop’s')
})
