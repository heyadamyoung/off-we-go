...
/* A photograph is filed at the itinerary item it was taken near, and that
   filing is a link rather than a place. The distinction is the whole story of
   this file: for a while the map drew a stop's photographs as one stack on the
   stop's own point, so being filed at the museum replaced where a picture was
   taken with where the museum is — the street outside it, the bikes, the
   family and the sky all collapsed onto one pin, and a picture that appeared
   where it was taken had moved after a reload. Unfiling everything fixed that
   and cost the gallery its entire shape. Drawing them at their own point fixed
   it properly, and the link came back.

   So nothing below reads or writes a coordinate. It answers one question —
   which stop, if any — and photo-cluster-core.ts answers the other.
