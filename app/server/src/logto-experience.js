...
export function createLogtoExperienceService({
  identityProvider,
  publicUrl,
  fetch: fetchFn = fetch,
  clock = () => new Date(),
  ...
  return {
    async start() {
      ...
      for (const [id, value] of interactions)
        if (value.expiresAt <= clock()) interactions.delete(id)
      return handle
    },

    event(handle) {
      return get(handle)?.event || null
    },

    async forward({ handle, method, path = '', body }) {
      const interaction = get(handle)
      if (!interaction)
        return { status: 401, body: JSON.stringify({ error: 'No active sign-in attempt' }) }
      ...
      let redirectTo = null
      try {
        redirectTo = JSON.parse(result.text).redirectTo
      } catch {}
      if (!redirectTo)
        return { status: 502, body: JSON.stringify({ error: 'Logto did not complete sign-in' }) }
      ...
      for (let hop = 0; hop < 8; hop++) {
        ...
        if (!location)
          return {
            status: 502,
            body: JSON.stringify({ error: 'Logto returned an invalid redirect' }),
          }
        ...
        if (candidate.origin === callback.origin && candidate.pathname === callback.pathname) {
          const identity = await identityProvider.exchangeCallback({
            ...
            codeVerifier: interaction.codeVerifier,
          })
      ...
      return {
        status: 502,
        body: JSON.stringify({ error: 'Logto sign-in took too many redirects' }),
      }
    },
  }
}
