const knownErrors = new Map([
  ...
  [
    ...
    ['auth.email_not_allowed', 'This email address cannot be used. Try a different email.'],
  ],
  [
    'session.email_blocklist.disposable_email_validation_failed',
    ['auth.email_not_allowed', 'Use a permanent email address to create your account.'],
  ],
  [
  ...
  [
    ...
    ['auth.attempt_expired', 'Your verification attempt expired. Request a new code.'],
  ],
  [
    'session.verification_expired',
    ['auth.attempt_expired', 'Your verification attempt expired. Request a new code.'],
  ],
  [
...
export function safeExperienceError(status, body, path = '') {
  let providerCode = ''
  try {
    providerCode = String(JSON.parse(String(body || '{}')).code || '')
  } catch {}
  const known = knownErrors.get(providerCode)
  if (known) return { code: known[0], error: known[1] }
  if (status === 429)
    return { code: 'auth.rate_limited', error: 'Too many attempts. Wait a moment, then try again.' }
  if (status >= 500) {
    return path === 'verification/verification-code'
      ...
      : {
          ...
          error: 'The sign-in service is temporarily unavailable. Please try again later.',
        }
  }
