...
test('a keyboard is the part of the page it covers', () => {
  ...
  assert.equal(keyboardInset({ height: 480, offsetTop: 20 }, 844), 344, 'a scrolled visual viewport counts')
})
...
test('small changes are not a keyboard', () => {
  ...
  assert.equal(keyboardInset({ height: 900, offsetTop: 0 }, 844), 0, 'a taller viewport is not a keyboard')
})
