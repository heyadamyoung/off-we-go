...
import { authCallbackMessage, authErrorMessage, authSuccessMessage } from '../src/auth-messages-core.ts'
...
test('auth errors turn provider codes into comprehensive actionable messages', () => {
  const cases = [
    ['user.email_already_in_use', 422, 'An account already exists for this email. Sign in instead.'],
...
test('auth errors use safe action-specific fallbacks instead of raw provider details', () => {
  assert.equal(authErrorMessage(failure('unknown.internal', 503), 'signin'),
    'Sign-in is temporarily unavailable. Please try again later.')
  assert.equal(authErrorMessage(failure('unknown.internal', 503), 'send-code'),
    'We could not send a verification code right now. Please try again later.')
  assert.equal(authErrorMessage(failure('unknown.internal', 400), 'register'),
...
test('auth callback errors are allow-listed instead of echoing URL text', () => {
  assert.equal(authCallbackMessage('Sign-in was cancelled or could not be completed'),
    'Sign-in was cancelled. Please try again when you are ready.')
  assert.equal(authCallbackMessage('The identity provider could not verify this sign-in'),
    'We could not verify your sign-in. Please try again.')
  assert.equal(authCallbackMessage('<img src=x onerror=alert(1)>'),
