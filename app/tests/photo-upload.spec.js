...
test('selected photo thumbnails reveal their location source and expected map placement', async ({
  ...
  await page.locator('.dlg input[type="file"]').evaluate(input => {
    const tagged = new File([new Uint8Array([0xff, 0xd8, 0xff, 0xd9])], 'tagged.jpg', {
      type: 'image/jpeg',
    })
    Object.defineProperty(tagged, 'offwegoMetadata', {
      value: {
        lng: -3.1883,
        lat: 55.9533,
    ...
    const untagged = new File([new Uint8Array([0xff, 0xd8, 0xff, 0xd9])], 'untagged.jpg', {
      type: 'image/jpeg',
    })
    Object.defineProperty(untagged, 'offwegoMetadata', {
      value: {
        takenAt: '2026-08-30T12:00:00.000Z',
      },
    })
    const transfer = new DataTransfer()
    transfer.items.add(tagged)
    transfer.items.add(untagged)
  ...
  await expect(
    page.getByText(
      'Trip history will be checked first; the current phone position shown here is the fallback.',
    ),
  ).toBeVisible()
})
