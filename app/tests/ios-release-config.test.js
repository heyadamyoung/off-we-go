...
test('the TestFlight export configuration uploads an automatically signed App Store build', async () => {
  const { createExportOptions } = await import('../scripts/iosReleaseCore.mjs');

  const plist = createExportOptions({ teamId: 'R65UN25Q64' });
...
test('the export configuration CLI emits the plist used by the release workflow', () => {
  const result = spawnSync(
    process.execPath,
    ['scripts/iosReleaseCore.mjs', 'R65UN25Q64'],
