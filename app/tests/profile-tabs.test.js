...
import {
  DEFAULT_PROFILE_TAB, PROFILE_TABS, PROFILE_TAB_LABELS, parseProfileSearch,
} from '../src/profile-tabs-core.ts'

test('every tab has a label, and every label a tab', () => {
  assert.deepEqual(PROFILE_TAB_LABELS.map(([tab]) => tab), [...PROFILE_TABS])
  for (const [, label] of PROFILE_TAB_LABELS) assert.ok(label.length, 'a tab with no name is not a tab')
})
