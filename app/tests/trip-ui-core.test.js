...
import {
  DEFAULT_PREFERENCES, readPreferences, toggleChannel,
} from '../src/preferences-core.ts'
...
const STOPS = [
  { id: 's1', name: 'Rijksmuseum', day: 'Sat 5 Sep', time: '09:30', status: 'done', note: 'The Night Watch' },
...
test('a day shows its own stops and the photographs taken at them, in order', () => {
  ...
  assert.deepEqual(items.map(item => item.id), ['s1', 'p1', 's2'])
...
test('a search looks across the whole trip, not only the chosen day', () => {
  ...
  assert.deepEqual(found.map(item => item.id), ['s3'])
})
...
test('a search matches a caption and a stop note as well as a name', () => {
  ...
  assert.deepEqual(byCaption.map(item => item.id), ['p2'])
...
test('the strip can leave photographs out without losing the stops', () => {
  ...
  assert.equal(items.every(item => item.kind === 'stop'), true)
...
test('a stop with no time still says something rather than showing an empty line', () => {
  ...
  assert.equal(stopItem({ id: 's', name: '' }).title, 'Untitled stop')
})
...
test('a preference added after somebody saved their settings still reads as its default', () => {
  ...
  assert.deepEqual(read.notify.digest, DEFAULT_PREFERENCES.notify.digest,
...
test('a channel a notification does not offer is discarded rather than stored', () => {
  const read = readPreferences({ notify: { arrivals: { on: true, channels: ['email', 'pigeon'] } } })
  assert.deepEqual(read.notify.arrivals.channels, ['push'],
...
test('an unknown discoverability setting does not widen who can find you', () => {
  assert.equal(readPreferences({ privacy: { discoverable: 'everyone' } }).privacy.discoverable,
    'shared')
})
