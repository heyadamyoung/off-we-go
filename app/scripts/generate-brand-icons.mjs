  ...
  for (const [density, sizes] of androidDensities) {
    ...
    await Promise.all([
      writeAsset(
        ...
        await opaqueIcon(sourcePath, sizes.launcher),
      ),
      writeAsset(
        path.join(directory, 'ic_launcher_round.png'),
        await roundLauncher(sourcePath, sizes.launcher),
      ),
      writeAsset(
        path.join(directory, 'ic_launcher_foreground.png'),
        await adaptiveForeground(source, sizes.foreground),
      ),
      writeAsset(
        path.join(directory, 'ic_launcher_monochrome.png'),
        await adaptiveForeground(monochrome, sizes.foreground),
      ),
    ])
  }
}
