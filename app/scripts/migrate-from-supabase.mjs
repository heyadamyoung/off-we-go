...
async function downloadLegacy(row) {
  ...
  if (!projectUrl || !serviceKey) {
    throw new Error(
      'LEGACY_SUPABASE_URL and LEGACY_SUPABASE_SERVICE_ROLE_KEY are required for private photos',
    )
  }
...
async function convertPhoto(row) {
  ...
  manifest.files.push({
    legacyPath: row.storage_path || row.external_url,
    storagePath,
    bytes: display.length,
...
try {
  const data = {
    users: await rows(
      'select id,email,created_at from auth.users where email is not null order by created_at',
    ),
  ...
  manifest.counts.source = Object.fromEntries(
  ...
  if (dryRun) {
    ...
    const owners = new Map(
      data.members
        ...
        .map(value => [value.trip_id, value.user_id]),
    )
    ...
    for (const photo of data.photos) {
      try {
        importedPhotos.push({ ...photo, ...(await convertPhoto(photo)) })
      } catch (error) {
        const skipped = {
          id: photo.id,
          source: photo.storage_path || photo.external_url || null,
          error: error.message,
        }
    ...
    try {
      for (const value of data.users) {
        await target.query(
          `insert into users(id,email,created_at) values($1,lower($2),$3)
          on conflict(id) do update set email=excluded.email`,
          [value.id, value.email, value.created_at],
        )
        const rawBase = slugBase(
          profileNames.get(value.id) || value.email.split('@')[0],
          'traveller',
          30,
        )
        const base =
      ...
      for (const value of data.trips) {
        ...
        await target.query(
          `insert into trips(id,slug,title,crew,dates,day_count,created_at)
        ...
        if (value.slug && value.slug !== slug) {
          await target.query(
            `insert into trip_slug_aliases(slug,trip_id) values($1,$2)
            on conflict(slug) do nothing`,
            [value.slug, value.id],
          )
        }
      }
      for (const value of data.members.filter(
        value => knownUsers.has(value.user_id) && knownTrips.has(value.trip_id),
      )) {
        await target.query(
          `insert into trip_members(trip_id,profile_id,role,joined_at)
      ...
      for (const value of data.invites.filter(value => knownTrips.has(value.trip_id))) {
        await target.query(
          `insert into trip_invites(id,trip_id,email,name,role,claimed_at,created_at)
      ...
      for (const value of data.stops.filter(value => knownTrips.has(value.trip_id))) {
        await target.query(
          `insert into stops(id,trip_id,name,kind,icon,day,time,lng,lat,status,note,image_url,source_url,seq,created_at)
      ...
      for (const value of importedPhotos) {
        ...
        await target.query(
          `insert into photos(id,trip_id,stop_id,user_id,lng,lat,caption,taken_by,taken_at,location_source,storage_path,thumb_path,seq,created_at)
          ...
          [
            ...
            value.trip_id,
            value.stop_id,
            owners.get(value.trip_id) || null,
            coordinates.lng,
            coordinates.lat,
            value.caption,
            value.taken_by,
            legacyDate(value.taken_at),
            coordinates.lng != null ? 'manual' : null,
            value.storagePath,
            value.thumbPath,
            value.seq || 0,
            value.created_at,
      ...
      for (const value of data.route.filter(value => knownTrips.has(value.trip_id))) {
        await target.query(
          `insert into route_points(trip_id,lng,lat,seq) values($1,$2,$3,$4)
          on conflict(trip_id,seq) do update set lng=excluded.lng,lat=excluded.lat`,
          [value.trip_id, value.lng, value.lat, value.seq],
        )
      }
      for (const value of data.comments.filter(
        value => photoIds.has(value.photo_id) && knownUsers.has(value.user_id),
      )) {
        await target.query(
          `insert into comments(id,trip_id,photo_id,user_id,body,created_at)
      ...
      for (const value of data.likes.filter(
        value => photoIds.has(value.photo_id) && knownUsers.has(value.user_id),
      )) {
        await target.query(
          `insert into photo_likes(trip_id,photo_id,user_id,created_at) values($1,$2,$3,$4)
          on conflict(photo_id,user_id) do nothing`,
          [value.trip_id, value.photo_id, value.user_id, value.created_at],
        )
      }
    ...
    for (const table of [
      ...
      targetCounts[table] = Number(
        (await target.query(`select count(*) count from ${table}`)).rows[0].count,
      )
    }
