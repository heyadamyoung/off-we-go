...
Each authentication user has one global `profiles` row containing their unique handle, display name, and avatar. New accounts choose a handle before email verification; handles are lowercase, 3–30 characters, and contain only letters, numbers, and single hyphens. Authenticated tripmates can open the profile at `/users/{handle}`; unrelated accounts cannot use the route to enumerate profiles. `trip_members` links a trip to a profile and contains only relationship data (`role` and `joined_at`); profile details are never copied into a membership. Updating `/api/profile` therefore changes the person everywhere they appear.

Trip slugs are allocated once from the trip title (`italy-2027`, then `italy-2027-2` when needed) and appear at `/trips/{slug}`. They do not change when the title is edited. Older opaque slugs and legacy `?t=` links remain resolvable and are replaced in the browser with the canonical human-readable path.
